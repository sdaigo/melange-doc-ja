...
拡張子が異なる限り、同じフィールドで複数のモジュールシステムを使うことができます。例えば、`(module_systems commonjs (es6 mjs))` は、CommonJS と拡張子 `.js` を使った JavaScript ファイルと、ES6 と拡張子 `.mjs` を使った JavaScript ファイルのセットを生成します。
