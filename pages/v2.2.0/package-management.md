...
At any given time, any opam switch can only install _at most_ a single version
...
- opam uses the concept of a _switch_
...
`opam upgrade` is also able to upgrade _all_ the packages of the local switch if
