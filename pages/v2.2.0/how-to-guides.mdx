...
この設定はライブラリレベルではなく、アプリケーションレベルで、`melange.emit`stanza の`module_systems`フィールドを使って行います。詳しくは[Build system](./build-system.mdx)のセクションを参照してください。
...
`package-specs`と同様、`melange.emit` stanza の`module_system`フィールドでアプリケーションレベルで設定します。[CommonJS や ES6 モジュール](./build-system.mdx#commonjs-または-es6-modules)のセクションを参照してください。
...
You can read more about this in the [Handling assets](./build-system.mdx#アセットを扱う) section.
